...
public final class PrintServiceLookupProvider extends PrintServiceLookup
    ...
    String[] lpcFirstCom = {
        "/usr/bin/lpc status | grep : | sed -ne '1,1 s/://p'",
        "/usr/bin/lpc status | grep -E '^[ 0-9a-zA-Z_-]*@' | awk -F'@' '{print $1}'"
    };

    String[] lpcAllCom = {
        "/usr/bin/lpc status all | grep : | sed -e 's/://'",
        "/usr/bin/lpc status all | grep -E '^[ 0-9a-zA-Z_-]*@' | awk -F'@' '{print $1}' | sort"
    };
    ...
    static int getBSDCommandIndex() {
        String command  = "/usr/bin/lpc status all";
    ...
    private PrintService getNamedPrinterNameBSD(String name) {
      ...
      String command = "/usr/bin/lpc status " + name + lpcNameCom[cmdIndex];
